...
generate_fsm(void)
{
	...
	fprintf(stderr,
		"%u keywords consumed %u states of %u max.\n",
		(u_int)COUNTOF(ntp_keywords),
		sst_highwater - 1,
		(u_int)COUNTOF(sst) - 1);

	printf("#define SCANNER_INIT_S %hu\n\n", initial_state);

	printf("const scan_state sst[%hu] = {\n"
	...
	for (i = 1; i < sst_highwater; i++) {
		...
		if (sst_highwater <= sst[i].match_next_s
			...
			fprintf(stderr,
				"keyword-gen internal error "
				"sst[%hu].match_next_s %hu too big\n",
		...
		if (sst_highwater <= sst[i].other_next_s
			...
			fprintf(stderr,
				"keyword-gen internal error "
				"sst[%hu].other_next_s %hu too big\n",
		...
		if (sst[i].finishes_token) {
			snprintf(token_id_comment,
				sizeof(token_id_comment), "%5hu %-17s",
				i, symbname(sst[i].finishes_token));
			if (i != sst[i].finishes_token) {
				fprintf(stderr,
					"keyword-gen internal error "
					"entry %hu finishes token %hu\n",
...
generate_token_text(void)
{
	...
	printf("#define LOWEST_KEYWORD_ID %hu\n\n", lowest_id);

	printf("const char * const keyword_text[%hu] = {", id_count);
...
)
{
	...
	return (int)p1->token - (int)p2->token;
}
...
)
{
	...
	if (token < COUNTOF(symb) && symb[token] != NULL) {
		...
		LIB_GETBUF(name);
		snprintf(name, LIB_BUFLENGTH, "%hu", token);
	}
